...
```bash
(curl -LOJ https://github.com/AirenSoft/OvenMediaEngine/archive/v0.10.1.tar.gz && tar xvfz OvenMediaEngine-0.10.1.tar.gz)
OvenMediaEngine-0.10.1/misc/prerequisites.sh
```
...
* `ws://<Server IP>:[<Signalling Port>/<Application name>/<Output Stream name>`
  ...
  * `<Application name>`: This value corresponds to `<Name>` of `<Application>` in `conf/Server.xml`. If you use the default configuration, you can use the `app`.
  * `<Output Stream name>`: You have to use an output stream name for streaming. If you are using the default configuration, an output stream named `<Stream Name>_o` , `<Stream Name>_webrtc` and `<Stream Name>_bypass` is automatically generated when stream is input. `_o` is an output stream that transcodes both video and audio. `_webrtc` is an output stream where video is passed through and only audio is transcoded into opus. `_bypass` is an output stream that bypass transcodes both video and audio. WebRTC does not support AAC as an audio codec, so if \_bypass is played with WebRTC, audio will be disabled.
...
`ws://192.168.0.1:3333/app/stream_webrtc`
{% endhint %}

{% hint style="info" %}
In addition, 

HLS streaming URL`will be` : 

`http://192.168.0.1/app/stream_o/playlist.m3u8`

`http://192.168.0.1/app/stream_bypass/playlist.m3u8`

MPEG-Dash streaming URL : 

`http://192.168.0.1/app/stream_o/manifest.mpd`

`http://192.168.0.1/app/stream_bypass/manifest.mpd`

Low-Latency MPEG-Dash streaming URL : 

`http://192.168.0.1/app/stream_o/manifest_ll.mpd`

`http://192.168.0.1/app/stream_bypass/manifest_ll.mpd`
{% endhint %}
