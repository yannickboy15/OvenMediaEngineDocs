...
```bash
(curl -LOJ https://github.com/AirenSoft/OvenMediaEngine/archive/v0.10.10.tar.gz && tar xvfz OvenMediaEngine-0.10.10.tar.gz)
OvenMediaEngine-0.10.10/misc/prerequisites.sh
```
...
If Hardware Encoder isn't installed on your PC, it's recommended to set x264 as follows: We highly recommend setting "bframes = 0" to reduce latency. Then set the "threads" option to 8 or less. Chrome doesn't handle more than 10 Nal Units. The best way to avoid this is to set "threads=8".

Keyframe Interval is the duration between full images sent by OBS, instead of just changes. The WebRTC player can only start playing at a keyframe, so a Keyframe Interval of 1 is recommended so that WebRTC will begin playing more quickly. This will increase the CPU and bandwidth requirements for your stream, so can be increased to 2 or 4 \(or higher\) if that is a problem.

CPU Usage Preset controls a number of presets. We recommend veryfast **with the x264 option aq-strength=0** - without the aq-strength=0 argument in x264 options, firefox cannot play streams via WebRTC \(though Chromium based browsers can\). Alternatively, you can use the ultrafast CPU Preset, though this preset will look much worse at lower bitrates compared to veryfast.

![Recommended x264 encoder settings for WebRTC playback via OME/OMP](.gitbook/assets/screen-shot-2021-02-07-at-11.14.14-pm.png)
